...
for idx, row in filtered_df.iterrows():
    ...
    with st.expander(f"**{row['PG Name']}** - {row['Shearing']} | <span class='pg-rent'>₹{row['Rent Price']}</span>", expanded=False):
        st.markdown(f"""
...
for idx, row in filtered_df.iterrows():
    folium.Marker(
        ...
        popup=f"<b>{row['PG Name']}</b><br>Shearing: {row['Shearing']}<br>Rent: ₹{row['Rent Price']}",
        tooltip=row["Area"],
