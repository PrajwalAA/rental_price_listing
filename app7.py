...
def main():
    ...
    with tab1:
        ...
        # Sidebar for residential filters
        st.sidebar.subheader("🔍 Residential Search Filters")
        ...
        residential_search_mode = st.sidebar.radio(
            "Select Search Mode",
        ...
        # Simple Search Mode for Residential
        if residential_search_mode == "Simple Search":
            ...
            if quick_search == "Rent Price":
                ...
                if rent_option == "Below budget":
                    max_rent = st.sidebar.number_input("Maximum rent (₹)", min_value=1000, value=20000, step=1000, key="residential_max_rent")
                    st.session_state.residential_filters["rent"] = f"below {max_rent}"
                elif rent_option == "Above budget":
                    min_rent = st.sidebar.number_input("Minimum rent (₹)", min_value=1000, value=10000, step=1000, key="residential_min_rent")
                    st.session_state.residential_filters["rent"] = f"above {min_rent}"
                elif rent_option == "Exact amount":
                    exact_rent = st.sidebar.number_input("Exact rent (₹)", min_value=1000, value=15000, step=1000, key="residential_exact_rent")
                ...
                area = st.sidebar.selectbox("Select area in Nagpur", residential_unique_values["areas"], key="residential_area")
                st.session_state.residential_filters["area"] = area
                
            elif quick_search == "Property Type":
                prop_type = st.sidebar.selectbox("Select property type", residential_unique_values["property_types"], key="residential_prop_type")
                st.session_state.residential_filters["property_type"] = prop_type
                
            elif quick_search == "Bedrooms":
                bedrooms = st.sidebar.slider("Number of bedrooms", 1, 5, 2, key="residential_bedrooms")
                st.session_state.residential_filters["bedrooms"] = str(bedrooms)
        
        # Advanced Search Mode for Residential
        elif residential_search_mode == "Advanced Search":
            st.sidebar.subheader("Advanced Filters")
            
            # Allow user to select multiple filters
            selected_filters = st.sidebar.multiselect(
                "Select filters to apply",
                list(residential_search_map.values())[:-1],  # Removed proximity_points and amenities_list
                default=["rent", "area"],
        ...
        # Compare Properties Mode for Residential
        ...
        # Apply filters button for Residential
        if st.sidebar.button("Apply Residential Filters", type="primary"):
            st.session_state.apply_residential_filters = True
        else:
            st.session_state.apply_residential_filters = False
        
        # Reset filters button for Residential
        if st.sidebar.button("Reset Residential Filters"):
            st.session_state.residential_filters = {}
            st.session_state.apply_residential_filters = False
            st.rerun()
    ...
    with tab2:
        ...
        # Sidebar for commercial filters
        st.sidebar.subheader("🔍 Commercial Search Filters")
        ...
        commercial_search_mode = st.sidebar.radio(
            "Select Search Mode",
            ["Simple Search", "Advanced Search", "Compare Properties"],
            key="commercial_search_mode"
        )
        
        # Simple Search Mode for Commercial
        if commercial_search_mode == "Simple Search":
            st.sidebar.subheader("Quick Search")
            ...
            if quick_search == "Rent Price":
                ...
                if rent_option == "Below budget":
                    max_rent = st.sidebar.number_input("Maximum rent (₹)", min_value=1000, value=20000, step=1000, key="commercial_max_rent")
                    st.session_state.commercial_filters["max_rent"] = max_rent
                elif rent_option == "Above budget":
                    min_rent = st.sidebar.number_input("Minimum rent (₹)", min_value=1000, value=10000, step=1000, key="commercial_min_rent")
                    st.session_state.commercial_filters["min_rent"] = min_rent
                elif rent_option == "Exact amount":
                    exact_rent = st.sidebar.number_input("Exact rent (₹)", min_value=1000, value=15000, step=1000, key="commercial_exact_rent")
                ...
                area = st.sidebar.selectbox("Select area in Nagpur", ["Any"] + commercial_areas, key="commercial_area")
                if area != "Any":
                    st.session_state.commercial_filters["area"] = area
                
            elif quick_search == "Property Type":
                prop_type = st.sidebar.selectbox("Select property type", ["Any"] + commercial_property_types, key="commercial_prop_type")
                if prop_type != "Any":
                ...
                if size_option == "Below size":
                    max_size = st.sidebar.number_input("Maximum size (sqft)", min_value=100, value=2000, step=100, key="commercial_max_size")
                    st.session_state.commercial_filters["max_size"] = max_size
                elif size_option == "Above size":
                    min_size = st.sidebar.number_input("Minimum size (sqft)", min_value=100, value=1000, step=100, key="commercial_min_size")
                    st.session_state.commercial_filters["min_size"] = min_size
                elif size_option == "Exact size":
                    exact_size = st.sidebar.number_input("Exact size (sqft)", min_value=100, value=1500, step=100, key="commercial_exact_size")
        ...
        # Advanced Search Mode for Commercial
        elif commercial_search_mode == "Advanced Search":
            # Display current property count
            st.sidebar.markdown(f"📊 Showing all {len(st.session_state.filtered_commercial_properties)} properties")
            ...
            col1, col2 = st.sidebar.columns(2)
            with col1:
                min_size = st.number_input("Min", min_value=0, value=0, key="commercial_min_size_adv")
            with col2:
                max_size = st.number_input("Max", min_value=0, value=10000, key="commercial_max_size_adv")
            
            # 2. Carpet Area (sqft)
            st.sidebar.markdown("2. **Carpet Area (sqft)**")
            col1, col2 = st.sidebar.columns(2)
            with col1:
                min_carpet = st.number_input("Min", min_value=0, value=0, key="commercial_min_carpet")
            with col2:
                max_carpet = st.number_input("Max", min_value=0, value=10000, key="commercial_max_carpet")
            
            # 3. Age of Property
            st.sidebar.markdown("3. **Age of Property**")
            col1, col2 = st.sidebar.columns(2)
            with col1:
                min_age = st.number_input("Min", min_value=0, value=0, key="commercial_min_age")
            with col2:
                max_age = st.number_input("Max", min_value=0, value=50, key="commercial_max_age")
            
            # 4. Brokerage
            st.sidebar.markdown("4. **Brokerage**")
            brokerage = st.sidebar.multiselect("Select", ["Yes", "No"], key="commercial_brokerage")
            
            # 5. Property ID
            st.sidebar.markdown("5. **Property ID**")
            property_id = st.sidebar.text_input("Enter ID", key="commercial_property_id")
            
            # 6. Furnishing
            st.sidebar.markdown("6. **Furnishing**")
            furnishing = st.sidebar.multiselect("Select", ["Furnished", "Unfurnished"], key="commercial_furnishing")
            
            # 7. Security Deposit
            st.sidebar.markdown("7. **Security Deposit**")
            col1, col2 = st.sidebar.columns(2)
            with col1:
                min_deposit = st.number_input("Min", min_value=0, value=0, key="commercial_min_deposit")
            with col2:
                max_deposit = st.number_input("Max", min_value=0, value=1000000, key="commercial_max_deposit")
            
            # 8. Rent Price
            st.sidebar.markdown("8. **Rent Price**")
            col1, col2 = st.sidebar.columns(2)
            with col1:
                min_rent = st.number_input("Min", min_value=0, value=0, key="commercial_min_rent_adv")
            with col2:
                max_rent = st.number_input("Max", min_value=0, value=100000, key="commercial_max_rent_adv")
            
            # 9. Area
            st.sidebar.markdown("9. **Area**")
            area = st.sidebar.multiselect("Select", commercial_areas, key="commercial_area_adv")
            
            # 10. Zone
            st.sidebar.markdown("10. **Zone**")
            zone = st.sidebar.multiselect("Select", commercial_zones, key="commercial_zone")
            
            # 11. Floor Number
            st.sidebar.markdown("11. **Floor Number**")
            floor_no = st.sidebar.multiselect("Select", commercial_floor_nos, key="commercial_floor_no")
            
            # 12. Total Floors
            st.sidebar.markdown("12. **Total Floors**")
            col1, col2 = st.sidebar.columns(2)
            with col1:
                min_total_floors = st.number_input("Min", min_value=0, value=0, key="commercial_min_total_floors")
            with col2:
                max_total_floors = st.number_input("Max", min_value=0, value=100, key="commercial_max_total_floors")
            
            # 13. Property Type
            st.sidebar.markdown("13. **Property Type**")
            property_type = st.sidebar.multiselect("Select", commercial_property_types, key="commercial_property_type_adv")
            
            # 14. Ownership
            st.sidebar.markdown("14. **Ownership**")
            ownership = st.sidebar.multiselect("Select", commercial_ownerships, key="commercial_ownership")
            
            # 15. Possession Status
            st.sidebar.markdown("15. **Possession Status**")
            possession_status = st.sidebar.multiselect("Select", commercial_possession_statuses, key="commercial_possession_status")
            
            # 16. Location Hub
            st.sidebar.markdown("16. **Location Hub**")
            location_hub = st.sidebar.multiselect("Select", commercial_location_hubs, key="commercial_location_hub")
            
            # 17. Facilities
            st.sidebar.markdown("17. **Facilities**")
            selected_facilities = st.sidebar.multiselect("Select", commercial_facilities, key="commercial_facilities")
            
            # 18. Lock-in Period
            st.sidebar.markdown("18. **Lock-in Period**")
            col1, col2 = st.sidebar.columns(2)
            with col1:
                min_lock_in = st.number_input("Min (months)", min_value=0, value=0, key="commercial_min_lock_in")
            with col2:
                max_lock_in = st.number_input("Max (months)", min_value=0, value=60, key="commercial_max_lock_in")
            
            # Action buttons
            st.sidebar.markdown("---")
            col1, col2 = st.sidebar.columns(2)
            with col1:
                if st.button("R. Reset all filters", key="commercial_reset"):
                    st.session_state.commercial_filters = {}
                    st.session_state.filtered_commercial_properties = commercial_properties_data
                    st.rerun()
            with col2:
                if st.button("V. View current properties", key="commercial_view"):
        ...
        # Compare Properties Mode for Commercial
        else:  # Compare Properties
            st.sidebar.subheader("Property Comparison")
            property_ids = st.sidebar.text_input(
                "Enter property IDs to compare (comma separated)",
        ...
        # Apply filters button for Commercial
        if commercial_search_mode != "Advanced Search":
            if st.sidebar.button("Apply Commercial Filters", type="primary"):
                # Build filters dictionary
                filters = {}
                ...
                # Apply filters
                st.session_state.commercial_filters = filters
                if commercial_search_mode == "Compare Properties" and "compare" in filters:
                    # For comparison mode, we don't filter the properties
                    st.session_state.filtered_commercial_properties = commercial_properties_data
                else:
                    st.session_state.filtered_commercial_properties = filter_commercial_properties(commercial_properties_data, filters)
        
        # Display current filters for Commercial
        if st.session_state.commercial_filters and commercial_search_mode != "Advanced Search":
            st.sidebar.subheader("Active Filters")
            for filter_type, value in st.session_state.commercial_filters.items():
